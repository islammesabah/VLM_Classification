"""
Clean implementation of traffic sign classification inference with VLM.
...
from Data.deepseek_tree_GTSRB import build_traffic_sign_tree
from Data.cifar_tree import build_cifar_tree
from data_loading import CIFAR_load_images, GTSRB_load_images

# read the list from openrouter_providers.json
with open('openrouter_providers.json', 'r') as f:
...
class InferenceConfig:
    ...
    def from_file(cls, config_path: str, dataset_name: str, model: str) -> 'InferenceConfig':
        ...
        try:
            ...
            with open(f'{dataset_name}_prompts.json', 'r') as f:
...
class DatasetLoader(ABC):
    ...
    @abstractmethod
    def load_images(self, config: InferenceConfig) -> Tuple[List, List[int], dict[str, str], Any]:
...
class VLMInference:
    ...
    def __init__(self, config: InferenceConfig, include_memory: bool = False,
        ...
        self.db_manager = DatabaseManager(f"Results/db/{config.database_name}")
    ...
    def zero_shot_inference(self, base64_image: str, class_names: Dict[str, str], config: dict[str, Any], include_description: int, short_description: str="", RunId: int=0) -> InferenceResult:
        ...
        if include_description == 2:
            prompt_description = f"""You are a visual dataset analyst helping another AI system learn to classify images.
            Your task is to analyze the image and provide a detailed description of its contents.
            Please describe the image in detail, including any objects, actions, or scenes that are present.
            Use the following format for your response:
            Image Description: <your description here>

            The description of the classes is as follows:
            Detailed description of the classes: {short_description}
            """
            messages_descr = [
                {
                    "role": "user",
        ...
        print(f"Zero-shot inference prompt: {prompt}")
        messages[0]["content"].append({
            "type": "text",
            "text": prompt
        })
    ...
    def tree_inference(self, base64_image: str, include_memory: bool, include_description: int, class_names: Dict[str, str], long_description: str="", short_description: str="") -> InferenceResult:
        ...
        if include_description == 1:
            prompt = f"""You are a visual dataset analyst helping another AI system learn to classify images.
            Your task is to analyze the image and provide a detailed description of its contents.
            Please describe the image in detail, including any objects, actions, or scenes that are present.
            Use the following format for your response:
            Image Description: <your description here>
    ...
    def run_inference(self, enable_tree: bool = False, include_memory: bool = False, include_description: int = 0, include_zero_shot_label: bool = False, run_id: int = 0):
        ...
        print(f"Loaded {len(images)} images from dataset '{self.config.dataset_name}'")
        ...
        # Process each image
        for image_idx in tqdm(range(len(images)), desc="Processing images"):
            ...
            if self.db_manager.image_exists(image_path, self.config.model, include_memory, include_description, include_zero_shot_label, self.config.temperature, run_id):
                print(f"Image {image_path} already processed for {self.config.model}. Skipping.")
                continue
            ...
            if enable_tree:
                ...
                # update the database with tree result
                # self.db_manager.update_row_tree(
                #     image_path=image_path,
                #     model_name=self.config.model,
                #     tree_result=tree_result.response,
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=['gpt-4o', 'qwen-vl-max', 'meta-llama/llama-3.2-11b-vision-instruct', 'meta-llama/llama-3.2-90b-vision-instruct'],
...
def main():
    ...
    try:
        ...
        # sys.stdout = open(f"Results/logs/{args.dataset_name}_{args.model.replace('/', '_')}_{config.temperature}_inference_{time.time()}.log", "w")
