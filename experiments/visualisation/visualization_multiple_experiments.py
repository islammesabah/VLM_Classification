...
sys.stdout = open(f'visualization_log_experiments_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt', 'w')

def load_db(dataset):
    dataset_to_db = {
        'CIFAR10': 'Results/db/cifar10_answers.db',
        'GTSRB': 'Results/db/answers.db'
    }
...
def main():
    ...
    # {"GTSRB": {"Llama 3.2 11B Vision Instruct": {"memory_True": {"temperature_0": {"tree": 20, "without_tree": 10, "Same": 5}
